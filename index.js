...
function createDictionary(entries) {
  ...
  const map = new Map();
  entries.forEach((value) => map.set(value[0], value[1]));
  return map;
}
...
function setValue(dictionary, key, value) {
  ...
  if (dictionary.has(key)) {
    dictionary.set(key, value.toString().toUpperCase());
  } else {
...
function convertDictionaryToSet(dictionary) {
  ...
  let result = new Set();
  for (const value of dictionary.keys()) {
    if (typeof value === "string") result.add(value.toUpperCase());
    if (typeof value === "number") result.add(value + 1);
  }
  return result;
}
...
function convertSetToDictionary(set) {
  ...
  for (const value of set) {
    result.set(value, value.toString().codePointAt(0));
  }
  return result;
}
